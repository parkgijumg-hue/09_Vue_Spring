...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    // -> 하나의 HTTP 요청(Request)당 한 번만 실행되는 Spring Security 필터
    // -> JWT 토큰 검증처럼 요청마다 한 번만 수행해야 하는 작업에 주로 사용
    ...
    protected void doFilterInternal(
        ...
        // Authorization 헤더가 존재하고, "Bearer " 로 시작하는 JWT 인증 방식 확인
    ...
    private Authentication getAuthentication(String token) {
        ...
        // - principal                : 로그인한 사용자 정보 (UserDetails)
        // - null                     : JWT 인증에서는 비밀번호가 필요 없으므로 null
        // - principal.getAuthorities : 사용자 권한 목록

        // UsernamePasswordAuthenticationToken : 인증 정보를 저장하는 객체
